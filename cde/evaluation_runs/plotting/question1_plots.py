...
for estimator, n_centers in [("MixtureDensityNetwork", 10), ("KernelMixtureNetwork", 20)]:
  ...
  plt.savefig("%s_%i_x_noise.png"%(estimator, n_centers))
  plt.clf()
...
for estimator, n_centers in [("MixtureDensityNetwork", 10), ("KernelMixtureNetwork", 20)]:
  ...
  plot_dict = dict([(simulator,
      {
        "y_noise_std=0.4": {"estimator": estimator, "y_noise_std": 0.4, "x_noise_std": None, "n_centers": n_centers, "simulator": simulator},
  ...
  plt.suptitle(title, fontsize=TITLE_SIZE)
  plt.tight_layout(h_pad=2, rect=[0, 0, 1, 0.95])
  plt.savefig("%s_%i_y_noise.png"%(estimator, n_centers))
  plt.clf()
...
y_noise_vals = list(reversed([None, 0.1, 0.2, 0.4]))
...
for estimator, n_centers in [("MixtureDensityNetwork", 10), ("KernelMixtureNetwork", 20)]:
  ...
  plt.savefig("%s_%i_%iobs_xy_noise_heatmap.png" % (estimator, n_centers, n_samples))